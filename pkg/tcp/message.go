...
type Message struct {
	MailBoxID int // If the message is a private message, MailBoxID is the friend user ID
	Type      int
	UserID    int
	UserName  string
	Content   []byte
...
func (m *Message) String() string {
	return fmt.Sprintf("MailboxID: %d, Type: %d, UserID: %d, UserName: %s, Content: %s", m.MailBoxID, m.Type, m.UserID, m.UserName, m.Content)
}
